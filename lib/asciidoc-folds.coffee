...
module.exports = AsciidocFolds =
  ...
  activate: (state) ->
    ...
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:dwim-toggle': (event) => @dwimtoggle(event)
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:toggle': => @toggle()
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:cycle': => @cycle()
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:togglefenced': => @togglefenced()
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:cycleall': => @cycleall()
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:toggleallfenced': => @toggleallfenced()
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:unfoldall': => @unfoldall()
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:foldall-h1': => @foldall(/^(=+)/)
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:foldall-h2': => @foldall(/^(==+)/)
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:foldall-h3': => @foldall(/^(===+)/)
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:foldall-h4': => @foldall(/^(====+)/)
    @subscriptions.add atom.commands.add 'atom-workspace', 'asciidoc-folds:foldall-h5': => @foldall(/^(=====+)/)
  ...
  cycleall: ->
    editor = atom.workspace.getActiveTextEditor()
    if typeof editor.__asciidocfolds_nextaction == "undefined"
      action = 'fold'
    else
      action = editor.__asciidocfolds_nextaction
    if action == 'fold'
      @foldall(/^=+\s/)
    ...
    if action == 'fold'
      editor.__asciidocfolds_nextaction = 'showheadings'
    else if action == 'showheadings'
      editor.__asciidocfolds_nextaction = 'unfold'
    else
      editor.__asciidocfolds_nextaction = 'fold'
  ...
  toggleallfenced: ->
    editor = atom.workspace.getActiveTextEditor()
    if typeof editor.__asciidocfolds_nextfencedaction == "undefined"
      action = 'fold'
    else
      action = editor.__asciidocfolds_nextfencedaction
    ...
    for row in [0..editor.getLastBufferRow()]
      ...
      if lookingforfirst
        ...
        if isend
          ...
          lookingforfirst = true
    if action == 'fold'
      editor.__asciidocfolds_nextfencedaction = 'unfold'
    else
      editor.__asciidocfolds_nextfencedaction = 'fold'
