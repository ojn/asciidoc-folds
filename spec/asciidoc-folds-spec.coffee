AsciidocFolds = require '../lib/asciidoc-folds'
...
describe "AsciidocFolds", ->
  ...
  beforeEach ->
    ...
    waitsForPromise ->
      atom.workspace.open('test.adoc')

    waitsForPromise ->
      Promise.all [
        atom.packages.activatePackage('language-gfm')
        atom.packages.activatePackage('asciidoc-folds')
        atom.commands.dispatch workspaceElement, 'asciidoc-folds:unfoldall'
      ]

  describe "when not folding anything", ->
    it "it should be 14 rows in both screen and buffer", ->
      editor = atom.workspace.getActiveTextEditor()
      expect(editor.getPath()).toContain 'test.adoc'
  ...
  describe "when folding all h1", ->
    it "it should be 4 rows on screen", ->
      editor = atom.workspace.getActiveTextEditor()
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:foldall-h1'
      expect(editor.getPath()).toContain 'test.adoc'
      expect(editor.getLineCount()).toBe 14
  ...
  describe "when folding all h2", ->
    it "it should be 8 rows on screen", ->
      editor = atom.workspace.getActiveTextEditor()
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:foldall-h2'
      expect(editor.getPath()).toContain 'test.adoc'
      expect(editor.getLineCount()).toBe 14
  ...
  describe "when folding all h3", ->
    it "it should be 12 rows on screen", ->
      editor = atom.workspace.getActiveTextEditor()
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:foldall-h3'
      expect(editor.getPath()).toContain 'test.adoc'
      expect(editor.getLineCount()).toBe 14
  ...
  describe "when moving to and folding h2-1", ->
    it "it should be 9 rows on screen", ->
      ...
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:toggle'
      expect(editor.getPath()).toContain 'test.adoc'
      expect(editor.getLineCount()).toBe 14
  ...
  describe "when moving to and cycling h1-1", ->
    it "it should be 5 rows on screen then 9 then 14 as cycled", ->
      editor = atom.workspace.getActiveTextEditor()
      editor.setCursorBufferPosition([0,0])
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:cycle'
      expect(editor.getPath()).toContain 'test.adoc'
      expect(editor.getLineCount()).toBe 14
      expect(editor.getScreenLineCount()).toBe 5
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:cycle'
  ...
  describe "when cycling the whole buffer", ->
    it "it should be 4 rows on screen then 8 then 14 as cycled", ->
      editor = atom.workspace.getActiveTextEditor()
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:cycleall'
      expect(editor.getPath()).toContain 'test.adoc'
      expect(editor.getLineCount()).toBe 14
      expect(editor.getScreenLineCount()).toBe 4
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:cycleall'
  ...
  describe "when cycling the whole buffer and there is no h1", ->
    beforeEach ->
      waitsForPromise ->
        atom.workspace.open('test-no-h1.adoc')
    it "it should cycle everything regardless", ->
      editor = atom.workspace.getActiveTextEditor()
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:cycleall'
      expect(editor.getPath()).toContain 'test-no-h1.adoc'
      expect(editor.getLineCount()).toBe 14
  ...
  describe "when toggling all fenced blocks", ->
    beforeEach ->
      waitsForPromise ->
        atom.workspace.open('test-fenced.adoc')
    it "it should be 17 then 27 lines on screen", ->
      editor = atom.workspace.getActiveTextEditor()
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:toggleallfenced'
      expect(editor.getPath()).toContain 'test-fenced.adoc'
  ...
  describe "when toggling the first fenced block", ->
    ...
    it "it should be 20 then 23 lines on screen", ->
      ...
      atom.commands.dispatch workspaceElement, 'asciidoc-folds:dwim-toggle'
      expect(editor.getPath()).toContain 'test-fenced.adoc'
      expect(editor.getLineCount()).toBe 27
